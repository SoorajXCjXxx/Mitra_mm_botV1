const botToken = `7005262009:AAHWZ2S7Cgb1_j1IUrC8DP5_Ot6Vk_WOecU`;
...
async function sendTips(chatId) {
    try{
        let tips = fs.readFileSync("tips.json", "utf-8");
        tips = JSON.parse(tips);
        tips = tips.tips;
        let random = Math.floor(Math.random() * tips.length);
        await bot.sendMessage(chatId, tips[random]);
    }
    catch(err){
        logError(bot,fs,err,ownerId);
    }
